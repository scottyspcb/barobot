...
public class Stk500V2 extends UploadProtocol{
    ...
    private static final boolean DEBUG_SHOW_READ            = true;
    private static final boolean DEBUG_SHOW_WRITE           = true;
    private static final boolean DEBUG_SHOW_COMMAND         = true;
    private static final boolean DEBUG_SHOW_COMMAND_STATUS  = true;
    private static final boolean DEBUG_SHOW_RECV            = true;
    private static final boolean DEBUG_SHOW_GETSYNC         = true;
    ...
    private IspCommunicator mComm;
    private AvrConf mAVRConf;
    private AvrMem mAVRMem;
    private CanLog Log;

    public Stk500V2(CanLog logger ) {
    	this.Log = logger;
    }
    ...
    private int drain() {
        ...
        while(true) {
            ...
            if(retval > 0) {
                ...
                if(DEBUG_SHOW_DRAIN) {
                    Log.d(TAG, "drain("+retval+") : " +Utils.toHexStr(buf[0]));
                }
            }
    ...
    private int command(byte[] buf, int len, int maxlen) {
        ...
        if(DEBUG_SHOW_COMMAND) { 
        	Log.d(TAG, "STK500V2.command("+Utils.toHexStr(buf, len)+", "+len+")"); 
        }
        ...
        while (bRetry) {
            ...
            if (DEBUG_SHOW_COMMAND_STATUS) {
                Log.d(TAG, "STK500V2.command(): status:"+status+",buf{"+Utils.toHexStr(buf, buf.length)+"}");
            }
...
//                        byte[] msgbuf = new byte[30];
    ...
    int recv(byte[] buf, int length) {
        ...
        while ((state != sDONE)) {
            ...
            if (DEBUG_SHOW_RECV) {
            	Log.d(TAG, "recv : "+Utils.toHexStr(c[0])); 
            }
            checksum ^= c[0];
            ...
            switch (state) {
                ...
                case sDATA:
                    if (DEBUG_SHOW_RECV) {
                    	Log.d(TAG, "recv | sDATA | msglen:"+msglen+", curlen:"+curlen+", length:"+length+", c[0]:"+Utils.toHexStr(c[0]));
                    	}
                    if (curlen < length) {
                        buf[curlen] = c[0];
                    } else {
