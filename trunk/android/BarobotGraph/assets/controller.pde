...
int dot_size	= 5;
int dimms		= 4;
int inited		= 0;		// ile wymiar�w danych
int skalax		= 4;
int interval_size= 0;
boolean need_refresh	= true;

//tablice w zale�nosci od wymiaru
int[] lastX		= new int[dimms];		// 10 wymiar�w max
int[] agvs		= new int[dimms];		// 10 wymiar�w max
...
void getValue( d, rawY ) {		// podaje liczb� px od do�u ekranu (do�u wykresu)
	//int y1	= dol - int(rawY * skala)-ymin;
	// ymin powinien by� zawsze na pozycji 0 wykresu czyli "dol" ekranu
	return dol + (rawY - ymin[d]) * skala[d];

	return dol + (rawY* skala[d] - ymin[d]* skala[d]);
}

void getValue2( d, rawY, margin ) {		// podaje liczb� px od do�u ekranu (do�u wykresu)
