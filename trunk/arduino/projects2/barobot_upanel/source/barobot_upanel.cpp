...
/*
  ...
  pwm_set_fade_time(-1, 1, 1);
  pwm_set_fade_time(0, 1, 1);
  pwm_set_fade_time(1, 2, 2);
  pwm_set_fade_time(2, 3, 3);
  pwm_set_fade_time(3, 4, 4);
  pwm_set_fade_time(4, 5, 5);
  pwm_set_fade_time(5, 6, 6);
  pwm_set_fade_time(6, 7, 7);
  pwm_set_fade_time(7, 0, 0);
	*/
  ...
  /*
    ...
    /*  boolean value  = digitalRead(in_buffer1[1]);
      byte ttt[1]    = {value ? 0xff:0xff};
			...
			DW(LED_TOP_RED, LOW);
			prog_mode = false;
		}else if( command == METHOD_RESET_NEXT ){         // Resetuj urz¹dzenie obok
			reset_next( LOW );
		}else if( command == METHOD_RUN_NEXT ){          // Koniec resetu urz¹dzenia obok, ustaw pin w stan wysokiej impedancji
			reset_next( HIGH );
...
void receiveEvent(int howMany){
	...
	// w tym miejscu jednynie bardzo proste komendy nie wymagaj¹ce zwrotek pracuj¹ce w funkcji obs³ugi przerwania, pamietac o volatile
}
...
void pwm_set_fade_time(uint8_t pin, uint8_t up, uint8_t down){
  if(pin == 0xff ){
    uint8_t i=COUNT_UPANEL_ONBOARD_LED;
    while(i--){
