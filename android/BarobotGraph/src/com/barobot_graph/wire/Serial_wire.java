...
public class Serial_wire implements Wire {
	...
	public boolean init() {
		mUsbManager = (UsbManager) BGraph.getInstance().getSystemService(
				Context.USB_SERVICE);
	...
	public void destroy() {
		...
		if (mPermissionReceiver_activated) {
			...
			try {
				BGraph.getInstance().unregisterReceiver(mPermissionReceiver);
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(ACTION_USB_PERMISSION)) {
				if (!intent.getBooleanExtra(
					...
					if (dev != null) {
						Log.w("serial",
								"Permission granted: " + dev.getVendorId());
						connectWith(dev);
					} else {
		...
		new AsyncTask<Void, Void, List<String>>() {
			...
			protected List<String> doInBackground(Void... params) {
				...
				for (final UsbDevice device : mUsbManager.getDeviceList()
						.values()) {
					final UsbSerialDriver driver = UsbSerialProber
							.probeSingleDevice(device);
					if (driver == null) {
						// Log.d("serial", "  - No UsbSerialDriver available.");
						// result.add(new DeviceEntry(device, null, 0));
					} else {
						// Log.d(TAG, "  + " + driver + ", " +
						// driver.getPortCount() + " ports.");

						Intent tt = BGraph.getInstance().registerReceiver(
								mPermissionReceiver,
						...
						if (!mUsbManager.hasPermission(device)) {
							final PendingIntent pi = PendingIntent
									.getBroadcast(BGraph.getInstance(), 0,
