...
import com.barobot_graph.BGraph;
import com.barobot_graph.Constant;
...
public class BluetoothChatService {
	...
	private synchronized void setState(int state) {
		// if (Constant.D) Log.d(Constant.TAG2, "setState() " + mState + " -> "
		// + state);
		mState = state;
	...
	public synchronized void start() {
		if (Constant.D)
			Log.d(Constant.TAG2, "synchronized start");
	...
	public synchronized void connect(BluetoothDevice device) {
		if (mState == Constant.STATE_CONNECTING) { // Cancel any thread
		...
		mConnectThread = new ConnectThread(device); // Start the thread to
													// connect with the given
													// device
	...
	public synchronized void connected(BluetoothSocket socket,
		...
		if (Constant.D)
			Log.d(Constant.TAG2, "connected, Socket Type:" + socketType);
		...
		setState(Constant.STATE_CONNECTED);																// ID
	}
	...
	public void destroy() {
		...
		try {
			BGraph.getInstance().unregisterReceiver(this.btEvents);
	...
	public void write(byte[] out) {
		...
		/*
		 * try { String str = new String(out, "UTF-8"); if (Constant.D)
		 * Log.d(Constant.TAG2, "Wysylam write: " + str); } catch
	...
	private void connectionFailed() {
		...
		/*
		 * Message msg = mHandler.obtainMessage(Constant.MESSAGE_TOAST); Bundle
		 * bundle = new Bundle(); bundle.putString(Constant.TOAST,
		 * "Nie dało się połączyć"); msg.setData(bundle);
		 * mHandler.sendMessage(msg);
		 */
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				// tmp =
				// device.createRfcommSocketToServiceRecord(Constant.MY_UUID_SECURE);
				Method m = device.getClass().getMethod("createRfcommSocket",
						new Class[] { int.class });
				tmp = (BluetoothSocket) m.invoke(device, 1);

				// } catch (IOException e) {
				// Constant.log(Constant.TAG2, "Socket Type: " + "Secure" +
				// "create() failed", e);
		...
		public void run() {
			...
			if (mAdapter.isDiscovering()) { // Always cancel discovery because
		...
		public void cancel() {
			try {
				...
				Constant.log(Constant.TAG2, "close() of connect " + "Secure"
						+ " socket failed", e);
			}
		}
	}
				...
				synchronized (ConnectedThread.this) {
					if (is_connected) {
						...
						try {
							...
							// mReader.run();
							mHandler.obtainMessage(Constant.MESSAGE_READ,
									bytes, -1, readMessage).sendToTarget(); // Send
																			// the
																			// obtained
																			// bytes
																			// to
							...
							connectionLost();
							BluetoothChatService.this.start(); // Start the
																// service over
																// to restart
																// listening
																// mode
	...
	public void connectBTDeviceId(String address) {
		BluetoothDevice device = mAdapter.getRemoteDevice(address); // Get the
																	// BluetoothDevice
																	// object
